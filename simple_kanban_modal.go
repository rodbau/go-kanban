...
func broadcastUpdate() {
	...
	for _, board := range activeBoards {
		...
		if currentBoard == nil {
			...
			fmt.Printf("⚠️ Board '%s' no longer exists, switching to default\n", board.CurrentBoard)
			board.CurrentBoard = "default"
			currentBoard = globalBoards["default"]
			if currentBoard == nil {
				// Load default board if not in memory
				currentBoard = loadBoardData("default")
				if currentBoard != nil {
		...
		if board.Dropdown != nil {
			...
			dropdownOptions := make([]components.DropdownOption, len(board.BoardsList))
			for i, boardName := range board.BoardsList {
				dropdownOptions[i] = components.DropdownOption{
					Value: boardName,
					Label: boardName,
				}
			}
			board.Dropdown.Options = dropdownOptions
		...
		func() {
			defer func() {
				if r := recover(); r != nil {
					fmt.Printf("⚠️ Client disconnected during broadcast: %v\n", r)
					boardIsActive = false
				}
			}()
			board.Commit()
			...
			<div style="display: flex; justify-content: center; align-items: center; gap: 15px;">
				<div style="display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.15); padding: 10px 20px; border-radius: 8px; backdrop-filter: blur(10px);">
					...
					<button 
						...
						{{else}}
							onclick="if(confirm('Are you sure you want to archive this board? It will be moved to the archived folder.')) { send_event('{{$.IdComponent}}', 'ArchiveBoard', '{{$.CurrentBoard}}'); }" 
	...
	<div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 20px;">
		<div class="modal-container" style="background: white; border-radius: 15px; width: 90%; max-width: 800px; max-height: 70vh; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
			...
			<div style="flex: 1; overflow-y: auto; padding: 20px 30px;">
											...
											<div style="display: flex; align-items: center; gap: 10px;">
												...
												<div>
													<a href="javascript:void(0);" 
													   onclick="downloadFile('{{$.CurrentBoard}}', '{{$.FormCardID}}', '{{.Name}}');" 
													   style="color: #3498db; text-decoration: none; font-weight: 500; cursor: pointer;">{{if .DisplayName}}{{.DisplayName}}{{else}}{{.Name}}{{end}}</a>
													<div style="font-size: 12px; color: #95a5a6;">{{.Size}} bytes</div>
												</div>
											</div>
											<button onclick="event.stopPropagation(); send_event('{{$.IdComponent}}', 'RemoveAttachment', '{{.ID}}')" 
													style="background: #e74c3c; color: white; border: none; border-radius: 4px; padding: 4px 8px; cursor: pointer; font-size: 12px;">×</button>
										</div>
										{{end}}
									</div>
								{{else}}
									<span style="color: #95a5a6; font-size: 13px;">No attachments yet</span>
								{{end}}
							</div>
							
							<!-- File Upload Area -->
							<div id="file-drop-zone-{{.IdComponent}}" 
								 style="border: 2px dashed #bdc3c7; border-radius: 6px; padding: 20px; text-align: center; cursor: pointer; transition: all 0.3s;"
								 ondragover="event.preventDefault(); this.style.borderColor='#3498db'; this.style.background='#f0f8ff';"
								 ondragleave="event.preventDefault(); this.style.borderColor='#bdc3c7'; this.style.background='transparent';"
								 ondrop="event.preventDefault(); this.style.borderColor='#bdc3c7'; this.style.background='transparent'; 
								...
								<div style="color: #95a5a6; font-size: 14px;">
									...
									<div style="font-size: 12px; margin-top: 5px;">Max 5MB per file</div>
								</div>
							</div>
							<input type="file" id="file-input-{{.IdComponent}}" 
								   multiple 
								   style="display: none;" 
				...
				{{else if eq .ModalType "new_board"}}
					<div style="display: flex; flex-direction: column; gap: 20px;">
						<div>
						...
						<div style="padding: 10px; background: #f8f9fa; border-radius: 6px;">
							<p style="margin: 0 0 10px 0; color: #7f8c8d; font-size: 13px;">
								<strong>💡 Tip:</strong> The new board will be created with default columns:
							</p>
							<div style="display: flex; gap: 10px;">
								<span style="padding: 4px 8px; background: #e3e8ef; border-radius: 4px; font-size: 12px;">To Do</span>
								<span style="padding: 4px 8px; background: #ffd4a3; border-radius: 4px; font-size: 12px;">In Progress</span>
								<span style="padding: 4px 8px; background: #a3e4d7; border-radius: 4px; font-size: 12px;">Done</span>
							</div>
						</div>
					</div>
				{{end}}
			...
			<div style="padding: 20px 30px; border-top: 1px solid #ecf0f1; display: flex; justify-content: space-between; align-items: center;">
				...
				<div style="display: flex; gap: 10px;">
					...
					<button onclick="send_event('{{.IdComponent}}', 'SaveModal', '')" 
						...
						💾 {{if eq .ModalType "new_board"}}Create Board{{else if or (eq .ModalType "add_card") (eq .ModalType "add_column")}}Add{{else}}Save{{end}}
					</button>
				</div>
			</div>
		</div>
